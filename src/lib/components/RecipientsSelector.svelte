  ...
  {#if chooseVisibility && editable}
    ...
    <div class="absolute top-10 left-6 right-6 sm:left-12 sm:right-12 z-50">
      <div class="max-w-lg space-y-2 text-sm sm:text-base">
        ...
        >
          {$t('common.private', { gender: 'male' })}
          <span class="text-sm">— {$t('common.journalAndDrafts')}</span>
        </button>
        <button
        ...
        >
          {$t('common.public', { gender: 'male' })}<span class="text-sm"
            >— {$t('common.everyoneCanRead')}</span
          >
        </button>
      </div>
    </div>
  ...
  {#if editable}
    <div class="relative border-gray-100 flex space-x-4 items-center py-2">
      <input
        ...
        placeholder={$t('common.enterEmailOrName')}
        type="text"
      />
    </div>

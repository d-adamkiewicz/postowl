	...
	<div class="max-w-screen-md mx-auto py-4 px-6">
		<NotEditable {editable}>
			<div class="flex items-center relative space-x-4">
				...
				<div class="flex-1" />
				{#if currentUser}
					<PrimaryButton size="sm" href="/posts/new">{$t('common.newPost')}</PrimaryButton>
				{/if}
				<button
...
{#if showMenu && !editable}
	<Modal on:close={() => (showMenu = false)}>
		<div class="p-8 flex flex-col space-y-4 relative">
			...
			{#if currentUser}
				...
				<div class="space-y-4 flex flex-col">
					{#if $page.url?.pathname !== '/friends'}
						<SecondaryButton size="sm" href="/friends">{$t('common.manageFriends')}</SecondaryButton>
					{/if}
				</div>
			{/if}
			...
			{#if currentUser}
				<div class="pt-8 flex">
					<div>{$t('common.signedAs')} {currentUser.name}</div>
					<div class="flex-1" />
					<div>
						<a
							...
							class="underline"
							href="/logout"
							on:click={toggleMenu}>{$t('common.signOut')}</a
						>
					</div>
				</div>
			{:else}
				<div class="">
					<form method="POST" action="/login" class="flex flex-col space-y-8">
						<div class="flex flex-col">
							<label for="password" class="font-semibold mb-6 text-3xl">{$t('common.signin')}</label>
							<Input
								type="password"
								name="password"
								id="password"
						...
						<div class="pt-8 text-sm sm:text-base">
							{@html $t('common.signInMesg', {
								value: `<a href="http://www.postowl.com" class="underline">PostOwl</a>`
							})}
						</div>
					</form>
				</div>

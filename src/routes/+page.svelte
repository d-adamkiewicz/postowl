...
<svelte:head>
	...
	<link href="/feed.xml" rel="alternate" type="application/rss+xml" title={name} />
		...
		<div class="space-y-4 flex flex-col">
			<SecondaryButton
				...
				on:click={() => {
					editable = true;
					showMenu = false;
				}}>{$t('common.editProfile')}</SecondaryButton
			>
		</div>
	{/if}
</WebsiteNav>
	...
	<div class="max-w-screen-md mx-auto px-6">
		...
		<div class="">
			<h1 class="text-center text-3xl md:text-6xl font-bold">
				<PlainText {editable} bind:content={name} placeholder={$t('common.enterText')}/>
			</h1>
		</div>
		<div class="prosa text-center py-4 sm:text-xl">
			<RichText {editable} bind:content={bio} placeholder={$t('common.enterText')} />
		</div>
	</div>
</div>
	...
	<div class="max-w-screen-md mx-auto px-6 pt-4 lg:pt-8">
		<div class={classNames(data.searchQuery ? '' : '', 'relative')}>
			...
			<input
				...
				placeholder={`${$t('common.searchInfo', { value: data.posts.length })}`}
				type="text"
			/>
			{#if currentUser}
				<div class="absolute inset-y-0 right-3 pt-[5px]">
					<select
						...
						on:change={onInput}
					>
						<option value="">{$t('common.showAll', { gender: 'other' })}</option>
						<option value="private">{$t('common.private', { gender: 'other' })}</option>
						<option value="public">{$t('common.public', { gender: 'other' })}</option>
					</select>
				</div>
			{/if}
		</div>
				...
				<div class="md:text-xl py-4 text-center">
					{#if !data.searchQuery && !searchFilter}
						{#if currentUser}
							...
							<a class="underline" href="#" on:click={() => (editable = true)}
								>{$t('common.personaliseThen_1')}</a
							>
							{$t('common.personaliseThen_2')}
							<a class="underline" href={'/posts/new'}>{$t('common.create_1')}</a>
							{$t('common.create_2')} 💌
						{:else}
							<!-- svelte-ignore a11y-invalid-attribute -->
							<a href="#" class="underline" on:click={() => (showMenu = true)}
								>{$t('common.signin')}</a
							>
							{$t('common.startWriting')}
						{/if}
					{:else}
						{$t('common.noLettersFound')}
					{/if}
				</div>
			</div>
		{/if}
